...
cvx_begin 
    ...
    variables  alpha(num_host,num_service) ...  
                    alpha_coef(num_host,num_service) ...
    ...
    alpha_coef == coef * ones(1,num_service) .* alpha;
      sum( alpha_coef ,1)' == sum(d .* (ones(num_service,1)*f_c'),2)
     ...
     sum(alpha,2)<=cap
      ...
      minimize sum(sum(c.*alpha)) +  r1 * pos( f_sla - f_c) 
